            ...
            GlassCard(padding: Brand.Space.l) {
                VStack(alignment: .leading, spacing: Brand.Space.m) {
                    ...
                    HStack {
                        ...
                        Spacer()
                        Button("OK") { dismiss() }.buttonStyle(GlassButtonStyle()).keyboardShortcut(.defaultAction)
                    }
                }
            }
...
struct UpdateBanner: View {
    ...
    var body: some View {
        HStack(spacing: Brand.Space.m) {
            Image(systemName: "arrow.down.circle.fill").foregroundStyle(Brand.Color.accent)
            VStack(alignment: .leading, spacing: 1) {
                Text("WhosBetter \(info.version) is available").font(Brand.Font.headline)
    ...
    var body: some View {
        ZStack {
            ...
            GlassCard(glow: true, padding: Brand.Space.l) {
                VStack(alignment: .leading, spacing: Brand.Space.m) {
                    Text("Unlock WhosBetter").font(Brand.Font.hero).foregroundStyle(Brand.Color.accentGradient)
                    ...
                    Button("Buy a license") { lic.openCheckout() }.buttonStyle(GlassButtonStyle())
                    Divider()
                    Text("Already have a key?").font(Brand.Font.headline)
                    HStack {
                        TextField("XXXX-XXXX-XXXX-XXXX", text: $key).textFieldStyle(.plain).font(Brand.Font.mono).glassField()
                        Button(busy ? "Activating…" : "Activate") {
                            busy = true
                            Task { await lic.activate(key: key); busy = false
                                   if case .licensed = lic.state { dismiss() } }
                        }.buttonStyle(GlassButtonStyle(prominent: false)).disabled(busy || key.isEmpty)
                    }
            ...
            VStack(alignment: .leading, spacing: Brand.Space.m) {
                GlassCard {
                    VStack(alignment: .leading, spacing: Brand.Space.s) {
                        ...
                        HStack {
                            SecureField("sk-ant-…", text: $apiKey).textFieldStyle(.plain).font(Brand.Font.mono).glassField()
                            Button(busy ? "Checking…" : "Save & test") { saveKey() }
                                .buttonStyle(GlassButtonStyle()).disabled(busy || apiKey.isEmpty)
                        }
                        if !status.isEmpty {
                ...
                GlassCard {
                    VStack(alignment: .leading, spacing: Brand.Space.s) {
                        ...
                        switch lic.state {
                        case .licensed(let k):
                            Label("Licensed · \(k)", systemImage: "checkmark.seal.fill").foregroundStyle(Brand.Color.win)
                            Button("Remove license from this Mac") { lic.deactivateLocally() }.buttonStyle(GlassButtonStyle(prominent: false))
                        case .trial(let d):
                ...
                Text("WhosBetter \(Bundle.main.object(forInfoDictionaryKey: "CFBundleShortVersionString") ?? "") (\(UpdateChecker.currentBuild))")
                    .font(Brand.Font.caption).foregroundStyle(.secondary)
            }
            .padding(Brand.Space.l)
        }
